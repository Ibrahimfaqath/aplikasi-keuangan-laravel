...
<head>
    ...
    <meta name="description" content="Edit transaksi di DompetKu.">
        ...
        <form action="{{ route('transactions.update', $transaction->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-2 gap-3 p-1 bg-slate-100 dark:bg-navy-800/80 rounded-xl border border-slate-200 dark:border-navy-700">
                    ...
                    <label class="relative flex items-center justify-center py-3 px-4 rounded-lg cursor-pointer has-[:checked]:bg-white dark:has-[:checked]:bg-slate-900 has-[:checked]:shadow-sm text-slate-600 dark:text-white/70 hover:text-slate-900 dark:hover:text-white transition">
                        <input type="radio" name="type" value="{{ $val }}" {{ $transaction->type == $val ? 'checked' : '' }} class="sr-only" required>
            ...
            <div class="bg-slate-50 dark:bg-navy-900 rounded-xl border border-slate-200 dark:border-navy-800 p-6 sm:p-8 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="hidden" name="category" id="category" value="{{ $transaction->category }}">
                    ...
                    @php
                        ...
                        $categories = $transaction->type === 'income' ? \App\Models\Transaction::INCOME_CATEGORIES : \App\Models\Transaction::EXPENSE_CATEGORIES;
                    @endphp

                    <div class="flex flex-wrap gap-2">
                        @foreach($categories as $cat)
                        <button type="button" data-category="{{ $cat }}" style="--cat-color: {{ $catColors[$cat] ?? '#6b7280' }}"
                                class="cat-chip px-3 py-2 rounded-lg border text-xs font-semibold bg-white dark:bg-navy-800 border-slate-200 dark:border-navy-700 text-slate-700 dark:text-white/80 hover:border-slate-300 dark:hover:border-navy-600 transition {{ $transaction->category == $cat ? 'active' : '' }}">
                            {{ $cat }}
                        </button>
                        @endforeach
                    </div>
                ...
                <div class="space-y-2">
                    ...
                    <!-- Preview -->
                    <div id="previewContainer" @if($transaction->image) @else class="hidden" @endif>
                        <img id="imagePreview" src="@if($transaction->image){{ asset('storage/' . $transaction->image) }}@endif" alt="Preview" class="w-full max-h-60 object-contain rounded-lg border border-slate-200 dark:border-navy-700">
                    </div>
