...
<body class="bg-slate-100 text-slate-800 font-sans antialiased">
    ...
    <div class="max-w-md mx-auto mt-10 p-6 bg-white rounded-xl shadow-md">
        ...
        <div class="mb-6 pb-3 border-b border-slate-200">
            ...
            <a href="/transactions" class="text-xs text-indigo-600 hover:underline">← Batal & Kembali</a>
        </div>

        <form action="/transactions/{{ $transaction->id }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <button type="submit" 
                    class="w-full bg-amber-600 hover:bg-amber-700 text-white font-medium py-2 px-4 rounded-lg transition duration-200 shadow text-sm">
                Update Transaksi
            </button>
        </form>
