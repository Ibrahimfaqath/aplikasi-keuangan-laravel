@php
    $title = 'Transaksi - DompetKu';
    $metaDescription = 'Kelola pemasukan, pengeluaran, dan anggaran bulanan dalam satu aplikasi pencatatan keuangan pribadi.';
    $containerClass = 'max-w-6xl space-y-6 sm:space-y-8 overflow-x-hidden';
@endphp

@extends('layouts.app')

@push('head')
    <!-- Chart.js hanya untuk halaman ini -->
...
<div x-data="dashboardApp()" x-init="initDashboard()">
    ...
    <!-- KARTU SALDO — saldo besar + tombol privasi di kanan, pemasukan/pengeluaran kecil di bawah -->
    <section class="relative overflow-hidden p-4 sm:p-6 bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 dark:from-navy-900 dark:via-navy-900 dark:to-navy-950 text-white rounded-2xl shadow-lg shadow-blue-900/25 dark:shadow-black/50 dark:border dark:border-navy-400/25">
        ...
        <!-- Konteks saldo: total seluruh transaksi tercatat -->
        <p class="relative mt-1 text-[11px] sm:text-xs font-medium text-blue-200/80 dark:text-navy-300/70">Total saldo seluruh transaksi tercatat</p>
        ...
        <div class="relative grid grid-cols-2 gap-2 sm:gap-3 mt-4 pt-4 border-t border-white/15">
            ...
            <div class="flex items-center gap-2.5 sm:gap-3 rounded-xl bg-white dark:bg-navy-800/80 border border-slate-200/80 dark:border-navy-700/60 px-2.5 sm:px-3 py-2.5 min-w-0">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/70">Pemasukan</p>
            ...
            <div class="flex items-center gap-2.5 sm:gap-3 rounded-xl bg-white dark:bg-navy-800/80 border border-slate-200/80 dark:border-navy-700/60 px-2.5 sm:px-3 py-2.5 min-w-0">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/70">Pengeluaran</p>
    ...
    <section class="bg-white dark:bg-navy-900 border border-slate-200/80 dark:border-navy-800 rounded-2xl p-4 sm:p-6 shadow-sm">
        ...
        @if($budget)
            ...
            <div class="flex items-end justify-between gap-3">
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-wider {{ $statusTxt }}">
                ...
                <div class="text-right flex-shrink-0">
                    ...
                    <p class="text-xs font-medium text-slate-500 dark:text-white/60">terpakai</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3 mt-5 pt-4 border-t border-slate-100 dark:border-navy-800">
                <div class="min-w-0">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-white/60">Batas</p>
                ...
                <div class="min-w-0">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-white/60">Terpakai</p>
                ...
                <div class="min-w-0">
                    <p class="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-white/60">Sisa / hari</p>
                    ...
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-navy-800/50 transition">
                        ...
                        <td class="py-4 px-6 text-center no-print">
                            <div class="inline-flex items-center gap-1.5">
                                <a href="{{ route('transactions.edit', $item->id) }}" class="inline-flex items-center justify-center w-10 h-10 text-slate-400 hover:text-blue-600 rounded-lg transition" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <button type="button" @click="confirmDeleteUrl = '{{ route('transactions.destroy', $item->id) }}'"
                                        class="inline-flex items-center justify-center w-10 h-10 text-slate-400 hover:text-rose-600 rounded-lg transition" title="Hapus">
        ...
        <div class="block md:hidden divide-y divide-slate-100 dark:divide-navy-800">
            ...
            <div class="p-4" x-data="{ open: false }">
                ...
                <div id="tx-actions-{{ $item->id }}" x-show="open" x-cloak
                    ...
                    @if(!empty($item->image))
                    <a href="{{ asset('storage/' . $item->image) }}" target="_blank" class="inline-flex items-center gap-1 px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-navy-800 dark:hover:bg-navy-700 text-slate-600 dark:text-white/80 rounded-lg text-xs font-semibold transition">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        <span>Bukti</span>
                    </a>
                    @endif
                    <a href="{{ route('transactions.edit', $item->id) }}" class="inline-flex items-center gap-1 px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-navy-800 dark:hover:bg-navy-700 text-slate-600 dark:text-white/80 rounded-lg text-xs font-semibold transition">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        <span>Edit</span>
                    </a>

                    <button type="button" @click="confirmDeleteUrl = '{{ route('transactions.destroy', $item->id) }}'"
                            class="inline-flex items-center gap-1 px-3 py-2 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-400 rounded-lg text-xs font-semibold transition">
    ...
    <div x-show="confirmDeleteUrl" x-cloak
         class="fixed inset-0 z-[60] flex items-center justify-center p-4"
         role="dialog" aria-modal="true" aria-labelledby="delete-modal-title">
        <div class="fixed inset-0 bg-slate-900/60 dark:bg-navy-950/80 backdrop-blur-sm" @click="confirmDeleteUrl = null"></div>
        <div class="relative w-full max-w-sm bg-white dark:bg-navy-900 rounded-2xl border border-slate-200 dark:border-navy-800 shadow-2xl p-6 text-center">
            <div class="w-12 h-12 mx-auto rounded-2xl bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400 flex items-center justify-center mb-3">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
            </div>
            <h3 id="delete-modal-title" class="text-base font-bold text-slate-900 dark:text-white">Hapus transaksi ini?</h3>
            <p class="text-xs text-slate-500 dark:text-white/70 mt-1">Tindakan ini tidak bisa dibatalkan.</p>
            <div class="flex items-center justify-center gap-3 mt-5">
                <button type="button" @click="confirmDeleteUrl = null"
                        class="px-4 py-2 bg-white dark:bg-navy-800 text-slate-700 dark:text-white/80 border border-slate-200 dark:border-navy-700 rounded-xl text-xs font-semibold hover:bg-slate-50 dark:hover:bg-navy-700 transition">
                    Batal
                </button>
                <form method="POST" :action="confirmDeleteUrl" class="inline">
                    @csrf @method('DELETE')
                    <button type="submit"
                            class="inline-flex items-center gap-1.5 px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-semibold shadow-md shadow-rose-600/20 transition">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        Hapus
                    </button>
                </form>
            ...
            <div class="relative transform overflow-hidden rounded-2xl bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-800 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md">
                ...
                <form action="{{ route('budgets.store') }}" method="POST" class="p-6 space-y-4" x-data="budgetForm({{ $budget?->amount ?? 0 }})">
                    ...
                    <div>
                        ...
                        <p class="mt-1.5 text-xs text-slate-400 dark:text-white/50">Ketik angka, otomatis diformat. Contoh: 1.500.000</p>
                    </div>

                    <!-- Chip nominal cepat -->
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-white/60 mb-2">Pilih cepat</p>
    ...
    function dashboardApp() {
        return {
            ...
            initDashboard() {
                ...
                // Data sudah dirender server-side — langsung inisialisasi grafik
                this.$nextTick(() => {
                    this.initTrendChart();
                    this.initCategoryChart();
                });
