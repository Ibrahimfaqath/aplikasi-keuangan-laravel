...
<body class="min-h-full bg-white dark:bg-black text-slate-900 dark:text-white font-sans antialiased flex flex-col">
    ...
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" x-transition
        ...
        <div class="ml-3 text-xs font-semibold text-slate-700 dark:text-neutral-100">{{ session('status') }}</div>
    ...
    <div class="flex-1 w-full max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('transactions.index') }}" class="p-2 bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl text-slate-500 dark:text-neutral-400 hover:bg-slate-50 dark:hover:bg-neutral-800 transition">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-neutral-400">Kelola informasi akun kamu.</p>
            </div>
        </div>

        <!-- Avatar Card -->
        <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-slate-200/80 dark:border-neutral-800 shadow-sm p-6 flex items-center gap-4">
            <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 dark:from-gold-400 dark:to-gold-500 dark:text-black flex items-center justify-center text-white text-2xl font-extrabold shadow-lg shadow-blue-500/20 flex-shrink-0">
                {{ strtoupper(substr(Auth::user()->name ?? 'U', 0, 1)) }}
            </div>
            <div class="min-w-0">
                <h2 class="text-base font-bold text-slate-900 dark:text-white truncate">{{ Auth::user()->name ?? 'Pengguna' }}</h2>
                <p class="text-xs text-slate-500 dark:text-neutral-400 truncate">{{ Auth::user()->email ?? '' }}</p>
            </div>
        </div>

        <!-- Form: Informasi Profil -->
        <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-slate-200/80 dark:border-neutral-800 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-neutral-800">
                ...
                <p class="text-xs text-slate-500 dark:text-neutral-400 mt-0.5">Perbarui nama dan email akun kamu.</p>
            </div>
            ...
            <form method="post" action="{{ route('profile.update') }}" class="p-6 space-y-5">
                ...
                <div>
                    <label for="name" class="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-neutral-400 mb-1.5">Nama Lengkap</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" required autofocus autocomplete="name"
                           class="w-full px-4 py-3 bg-slate-50 dark:bg-neutral-800/60 border border-slate-200 dark:border-neutral-700/80 rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                    @error('name') <p class="text-xs text-rose-600 dark:text-rose-400 mt-1.5">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="email" class="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-neutral-400 mb-1.5">Email</label>
                ...
                <div class="pt-3 border-t border-slate-100 dark:border-neutral-800 flex items-center gap-3">
                    <button type="submit" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 dark:bg-gold-400 dark:hover:bg-gold-300 dark:text-black text-white rounded-xl text-sm font-semibold shadow-md shadow-blue-600/20 transition">Simpan</button>
        ...
        <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-slate-200/80 dark:border-neutral-800 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-neutral-800">
                ...
                <p class="text-xs text-slate-500 dark:text-neutral-400 mt-0.5">Gunakan password yang kuat untuk keamanan akun.</p>
            </div>
            ...
            <form method="post" action="{{ route('password.update') }}" class="p-6 space-y-5">
                ...
                <div>
                    <label for="update_password_current_password" class="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-neutral-400 mb-1.5">Password Saat Ini</label>
                ...
                <div>
                    <label for="update_password_password" class="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-neutral-400 mb-1.5">Password Baru</label>
                ...
                <div>
                    <label for="update_password_password_confirmation" class="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-neutral-400 mb-1.5">Konfirmasi Password</label>
                    <input type="password" name="password_confirmation" id="update_password_password_confirmation" autocomplete="new-password" placeholder="Ulangi password baru"
                           class="w-full px-4 py-3 bg-slate-50 dark:bg-neutral-800/60 border border-slate-200 dark:border-neutral-700/80 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                </div>

                <div class="pt-3 border-t border-slate-100 dark:border-neutral-800 flex items-center gap-3">
                    <button type="submit" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 dark:bg-gold-400 dark:hover:bg-gold-300 dark:text-black text-white rounded-xl text-sm font-semibold shadow-md shadow-blue-600/20 transition">Perbarui Password</button>
        ...
        <!-- Hapus Akun -->
        <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-rose-200/60 dark:border-rose-900/30 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-rose-100 dark:border-rose-900/30">
                ...
                <p class="text-xs text-slate-500 dark:text-neutral-400 mt-0.5">Tindakan ini tidak dapat dibatalkan.</p>
            </div>
            <div class="p-6">
    ...
    <div x-data="{ show: false }" x-show="show" x-cloak x-on:open-modal.window="show = $event.detail === 'confirm-user-deletion'" x-on:close.window="show = false"
        ...
        <div class="fixed inset-0 bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm" x-on:click="show = false"></div>
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div x-show="show" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
                 ...
                 class="relative transform overflow-hidden rounded-2xl bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 text-left shadow-2xl sm:my-8 sm:w-full sm:max-w-lg">
                <form method="post" action="{{ route('profile.destroy') }}" class="p-6 sm:p-8">
                    @csrf @method('delete')
                    <h3 class="text-base font-bold text-slate-900 dark:text-white mb-2">Yakin ingin menghapus akun?</h3>
                    <p class="text-sm text-slate-500 dark:text-neutral-400 mb-5">Masukkan password untuk konfirmasi. Semua data akan dihapus permanen.</p>
                    <div>
                        <label for="password" class="sr-only">Password</label>
                        <input type="password" name="password" id="password" placeholder="Masukkan password"
                               class="w-full px-4 py-3 bg-slate-50 dark:bg-neutral-800/60 border border-slate-200 dark:border-neutral-700/80 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-500 transition">
                        @error('userDeletion.password') <p class="text-xs text-rose-600 dark:text-rose-400 mt-1.5">{{ $message }}</p> @enderror
                    </div>
                    <div class="flex items-center justify-end gap-3 mt-6">
                        <button type="button" x-on:click="show = false" class="px-4 py-2.5 bg-white dark:bg-neutral-800 text-slate-700 dark:text-neutral-300 border border-slate-200 dark:border-neutral-700 rounded-xl text-xs font-semibold hover:bg-slate-50 transition">Batal</button>
