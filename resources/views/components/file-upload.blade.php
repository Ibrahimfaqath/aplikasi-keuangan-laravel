...
<div x-data="{ 
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        <button type="button" 
                ...
                class="flex items-center justify-center gap-2 px-4 py-3 bg-blue-50 dark:bg-gold-400/10 hover:bg-blue-100 dark:hover:bg-gold-400/20 text-blue-600 dark:text-gold-400 rounded-xl border border-blue-200 dark:border-gold-400/20 font-semibold text-sm transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div id="drop-zone-{{ $name }}" 
         class="relative border-2 border-dashed border-slate-200 dark:border-neutral-800 hover:border-blue-400 dark:hover:border-blue-500 rounded-2xl p-6 text-center bg-slate-50/50 dark:bg-neutral-800/40 hover:bg-slate-50 dark:hover:bg-neutral-800/50 transition cursor-pointer hidden md:block"
         @dragover.prevent
        ...
        <div x-show="!hasFile && !preview" class="space-y-2">
            ...
            <p class="text-xs sm:text-sm font-semibold text-slate-700 dark:text-neutral-300">
                <span class="text-blue-600 dark:text-gold-400">Klik</span> atau tarik gambar ke sini
            </p>
            <p class="text-xs text-slate-400 dark:text-neutral-500">PNG, JPG, JPEG hingga 2MB</p>
        </div>

        <!-- Preview -->
        <div x-show="hasFile || preview" 
             class="flex items-center justify-between p-3 bg-white dark:bg-neutral-900 rounded-xl border border-slate-200 dark:border-neutral-800">
            <div class="flex items-center gap-3 min-w-0">
                <img :src="preview" alt="Preview" class="w-12 h-12 rounded-lg object-cover border border-slate-100 dark:border-neutral-800 flex-shrink-0">
                <div class="text-left min-w-0">
                    <p class="text-xs font-bold text-slate-800 dark:text-neutral-100 truncate" x-text="fileName || 'Foto tersimpan'"></p>
