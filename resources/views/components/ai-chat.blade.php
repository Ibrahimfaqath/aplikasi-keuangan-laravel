...
<div x-data="aiChat()" x-cloak class="fixed bottom-6 right-6 z-50">
    ...
    <button @click="toggleChat()" 
            class="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg shadow-indigo-500/30 flex items-center justify-center transition-all duration-200"
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0"
         ...
         class="absolute bottom-20 right-0 w-[360px] sm:w-[400px] bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl shadow-slate-900/10 dark:shadow-black/30 overflow-hidden flex flex-col"
         style="max-height: 520px;">

        <!-- Header -->
        <div class="px-5 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white flex items-center gap-3">
        ...
        <div x-ref="messages" class="flex-1 overflow-y-auto p-4 space-y-4" style="min-height: 280px; max-height: 340px;">
            ...
            <div class="flex gap-3">
                ...
                <div class="bg-slate-100 dark:bg-slate-800 rounded-2xl rounded-tl-md px-4 py-3 max-w-[85%]">
                    <p class="text-sm text-slate-700 dark:text-slate-300">Halo! 👋 Saya DompetKu AI. Saya bisa bantu analisis keuangan, saran budget, atau jawab pertanyaan seputar transaksi kamu.</p>
                </div>
            </div>

            <!-- Quick Actions -->
            <div class="flex flex-wrap gap-2">
                <button @click="sendMessage('Bagaimana ringkasan keuangan saya bulan ini?')" class="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 rounded-full text-xs font-semibold border border-indigo-200/60 dark:border-indigo-800/50 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 transition">
                    📊 Ringkasan bulan ini
                </button>
            ...
            <template x-for="(msg, i) in messages" :key="i">
                <div :class="msg.role === 'user' ? 'flex justify-end' : 'flex gap-3'">
                    <div x-show="msg.role === 'assistant'" class="w-7 h-7 rounded-lg bg-indigo-100 dark:bg-indigo-950/60 flex items-center justify-center flex-shrink-0">
                        <svg class="w-4 h-4 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
                    </div>
                    <div :class="msg.role === 'user' 
                        ? 'bg-indigo-600 text-white rounded-2xl rounded-tr-md px-4 py-3 max-w-[85%]' 
                        : 'bg-slate-100 dark:bg-slate-800 rounded-2xl rounded-tl-md px-4 py-3 max-w-[85%]'">
            ...
            <div x-show="loading" class="flex gap-3">
                ...
                <div class="bg-slate-100 dark:bg-slate-800 rounded-2xl rounded-tl-md px-4 py-3">
        ...
        <div class="px-4 py-3 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
            <form @submit.prevent="sendMessage(input)" class="flex items-center gap-2">
                <input type="text" x-model="input" placeholder="Tanya seputar keuangan..." 
                       class="flex-1 px-4 py-2.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                       :disabled="loading">
                <button type="submit" :disabled="loading || !input.trim()"
                        class="p-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl transition">
