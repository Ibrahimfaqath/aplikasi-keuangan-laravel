@php
    $title = 'DompetKu AI — Asisten Keuangan';
    $metaDescription = 'DompetKu AI — asisten keuangan pintar untuk analisis, saran hemat, dan baca struk otomatis.';
    $containerClass = 'max-w-full flex-1 min-h-0 flex flex-col !p-0';
@endphp
    ...
    <div class="w-full flex-1 min-h-0 flex flex-col"
            ...
            <div class="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
                ...
                <div class="flex justify-end" x-show="messages.length > 0" x-cloak>
                    <button type="button" @click="confirmClear = true"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-500 dark:text-white/70 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-full transition">
                    ...
                    <div class="text-center pt-10 sm:pt-16">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-8 max-w-lg mx-auto">
                            <button type="button" @click="sendMessage('Bagaimana ringkasan keuangan saya bulan ini?')"
                                ...
                                <p class="text-xs text-slate-500 dark:text-white/70 mt-0.5">Saldo, pemasukan, dan pengeluaran</p>
                            </button>
                            <button type="button" @click="sendMessage('Kategori mana yang paling banyak pengeluaran saya?')"
                                ...
                                <p class="text-xs text-slate-500 dark:text-white/70 mt-0.5">Lihat pengeluaran terbesar</p>
                            </button>
                            <button type="button" @click="sendMessage('Berikan tips menghemat pengeluaran saya')"
                                ...
                                <p class="text-xs text-slate-500 dark:text-white/70 mt-0.5">Saran mengelola pengeluaran</p>
                            </button>
                            <button type="button" @click="$refs.ocrInput.click()"
                                ...
                                <p class="text-xs text-slate-500 dark:text-white/70 mt-0.5">Foto struk, data otomatis terisi</p>
                            </button>
                        </div>
                    </div>
                </template>
                        ...
                        <div :class="msg.role === 'user'
                            ...
                            <div x-show="msg.transaction" class="mt-2.5 rounded-xl border border-slate-200 dark:border-navy-700 overflow-hidden bg-white dark:bg-navy-950/40">
                                <div class="px-3.5 py-2.5 space-y-1">
                                    ...
                                    <p class="text-xs text-slate-400 dark:text-white/50" x-text="msg.transaction?.transaction_date"></p>
                                </div>
                            ...
                            <div x-show="msg.ocrItems" class="mt-2.5 rounded-xl border border-slate-200 dark:border-navy-700 overflow-hidden bg-white dark:bg-navy-950/40">
                                <div class="px-3.5 py-2 border-b border-slate-100 dark:border-navy-800 flex items-center justify-between gap-2">
                                    <span class="text-xs font-bold text-slate-700 dark:text-white/80">Hasil scan — <span x-text="msg.ocrItems?.length"></span> item</span>
                                    <label class="flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-white/60 cursor-pointer select-none">
                                    ...
                                    <template x-for="(it, idx) in (msg.ocrItems || [])" :key="idx">
                                        <label class="flex items-start gap-2.5 px-3.5 py-2.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-navy-800/50 transition">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                ...
                                                <p class="text-xs text-slate-500 dark:text-white/60 mt-0.5 truncate" x-text="it.category + ' · ' + (it.type === 'income' ? 'Pemasukan' : 'Pengeluaran')"></p>
                                            </div>
                                        </label>
                                    </template>
                                </div>
        ...
        <div class="fixed inset-x-0 bottom-0 z-40 px-4 sm:px-6 pt-10 pb-24 md:pb-6 pointer-events-none">
            ...
            <div class="relative max-w-3xl mx-auto pointer-events-auto">
                ...
                <p x-show="recording" x-cloak class="text-xs text-rose-600 dark:text-rose-400 font-semibold mt-2 text-center flex items-center justify-center gap-1">
                    <span class="w-1.5 h-1.5 bg-rose-500 rounded-full animate-pulse inline-block"></span>
                    Mendengarkan... bicaralah sekarang
                </p>
                <p x-show="autoSending" x-cloak class="text-xs text-blue-600 dark:text-navy-300 font-semibold mt-2 text-center flex items-center justify-center gap-1">
        ...
        <!-- MODAL KONFIRMASI BERSIHKAN RIWAYAT -->
        <div x-show="confirmClear" x-cloak
             class="fixed inset-0 z-[60] flex items-center justify-center p-4"
             role="dialog" aria-modal="true" aria-labelledby="clear-modal-title">
            <div class="fixed inset-0 bg-slate-900/60 dark:bg-navy-950/80 backdrop-blur-sm" @click="confirmClear = false"></div>
            <div class="relative w-full max-w-sm bg-white dark:bg-navy-900 rounded-2xl border border-slate-200 dark:border-navy-800 shadow-2xl p-6 text-center">
                ...
                <h3 id="clear-modal-title" class="text-base font-bold text-slate-900 dark:text-white">Hapus seluruh riwayat chat AI?</h3>
                <p class="text-xs text-slate-500 dark:text-white/70 mt-1">Percakapan ini tidak bisa dikembalikan.</p>
                <div class="flex items-center justify-center gap-3 mt-5">
                    ...
                    <form method="POST" action="{{ route('ai.clear') }}" class="inline">
                        @csrf @method('DELETE')
                        <button type="submit"
